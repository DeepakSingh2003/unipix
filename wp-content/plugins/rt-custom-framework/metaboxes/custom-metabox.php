...
//tribe_events event post type metabox
add_action( 'cmb2_admin_init', 'unipix_kids_class_metabox' );
/**
 * Hook in and add a demo metabox. Can only happen on the 'cmb2_admin_init' or 'cmb2_init' hook.
 */
function unipix_kids_class_metabox() {
	$prefix = 'rt_'; 

	$cmb_project = new_cmb2_box( array(
		'id'            => $prefix . 'unipix-kids-class',
		'title'         => esc_html__( 'Unipix Kids Class Information', 'rs-framework' ),
		'object_types'  => array( 'tribe_events' ), // Post type		
		'priority'      => 'low',
	) );
	// Repeater Field for name and label
    $group_field_id = $cmb_project->add_field(array(
        'id'          => 'unipix_kids_class_info',
        'type'        => 'group',
        'options'     => array(
            'group_title'   => esc_html__('Label & Name {#}', 'rs-framework'), 
            'add_button'    => esc_html__('Add Another Project', 'rs-framework'),
            'remove_button' => esc_html__('Remove Project', 'rs-framework'),
            'sortable'      => true, 
        ),
    ));
    $cmb_project->add_group_field($group_field_id, array(
        'name' => esc_html__('Label', 'rs-framework'),
        'id'   => 'unipix_kids_class_label',
        'type' => 'text',
		  'default' => esc_html__('Class -', 'rs-framework'),
    ));
    $cmb_project->add_group_field($group_field_id, array(
        'name' => esc_html__('Name', 'rs-framework'),
        'id'   => 'unipix_kids_class_name',
        'type' => 'text',
		  'default' => esc_html__('Nursery (3-5 Years)', 'rs-framework'),
    ));
}
